...
`cmd/binman/` 是本仓库 OCI artifact 的消费端。它实现单个静态 Go 二进制 `bm`，面向没有 Nix、
...
1. **无宿主运行时依赖。** `bm` 必须保持 `CGO_ENABLED=0` 可构建，不调用外部
...
4. **digest 是版本标识。** 远端 OCI layer digest 与本地 `.binman-meta` 相同就跳过安装，
   `--force` 除外。
...
`bm` 自身使用 `binman-<arch>` tag，由 `.github/workflows/build-binman.yaml` 发布。bootstrap
...
- 普通包发布 workflow；
- `.github/workflows/build-binman.yaml`；
- `main.go`；
- `install.sh`；
- 根和本目录 `CLAUDE.md`。
...
- `install.sh`：仅用于首次安装 `bm` 的 bootstrap。
...
每个包完整解压到 `store/<name>/`。`.binman-meta` 位于包目录内部，随包一起创建和删除，格式为
缩进 JSON，字段包括：
...
根目录 `bin/`、`lib/`、`share/` 等按包内容建立相对 symlink；`.binman-meta` 永不参与 link。
profile 使用相同算法，但 link root 是 `<prefix>/profile/<name>/`。

下载 cache 位于 `${XDG_CACHE_HOME:-$HOME/.cache}/binman/<arch>/<name>.tar.gz`。cache 不是安装状态，
删除后不影响已装包。
...
- `sync [file]`：应用 YAML manifest，默认 `binman.yaml`；
  - `--force` 强制重装；
...
- `.binman-meta` 不得从远端归档继承，必须由 client 写入。

涉及解压或链接算法时，先补能复现边界的测试，再修改实现。

## Logging

每次调用把 slog text 写到 `<prefix>/binman.log`。终端显示关键步骤、下载进度和 summary；
`--verbose` 把详细日志镜像到 stderr。进度条走 stdout，避免与 verbose 日志冲突。

不要把日志文件当作状态源；命令行为只依赖 store 和 `.binman-meta`。
...
- 默认安装到 `~/.local/bin`；
- 接受 `BINMAN_INSTALL_DIR`、`BINMAN_ARCH`；
