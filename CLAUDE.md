...
Example — `eza-ls` (`packages/eza-ls`): an `ls`-compatible front-end backed by `eza`. The upstream `eza` manifest package is left untouched; `eza-ls` is a separate `common` package that ships the static `eza` binary plus a `bin/ls` bash wrapper (`packages/eza-ls/ls-wrapper.sh`). The wrapper invokes the co-located `./eza` (so `ls` is self-contained, not dependent on an `eza` on the host PATH), applies enhanced defaults (`--group-directories-first`/`--icons`/`--color`) only when stdout is a TTY (piped output stays plain for script parsers), and adds a light `ls`->`eza` flag compatibility layer: it splits bundled short-option clusters (e.g. `-lt`) into single letters and translates the ones whose meaning differs (`-h` dropped since eza is human-readable by default and its own `-h` is `--header`; `-t`/`-S`/`-U`/`-c`/`-u` mapped to eza `--sort=...`), passing every other short flag, long `--option`, and path straight through to eza.
