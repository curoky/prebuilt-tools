...
| `libarchive` | 🩹 本地 | ✅ | stock 静态 CLI 内嵌 OpenSSL output 和 libxml2 catalog 的 Nix store 路径；本地 override 关闭 XAR/libxml2，并把静态 OpenSSL 的配置、engine 与 module 默认目录改为系统路径，保留 ZIP AES 支持；darwin 尚未实机构建 | stock unstable 四个 CLI 均不内嵌 Nix store 路径，且最终 Mach-O 只依赖系统库并通过 TAR/ZIP/AES smoke test | — | `packages/libarchive/` |
